...
def start(message: types.Message):
    ...
    item1=types.KeyboardButton('Зарегестрироваться',request_contact=True)
    item2 =types.KeyboardButton('Отправить геоданные',request_location=True)
    markup.add(item1,item2)

    text='Хеллоу мээээн \n Для получения инфо , Зарегистрируйтесь'
...
def contact(message:types.Message):
    if message.contact is not None:
        ...
        if not is_user_exists(message.chat.id):
            insert_user(phone_number=message.contact.phone_number,first_name=message.contact.first_name,
            last_name=message.contact.last_name,chat_id=message.chat.id)
...
def location(message:types.Message):
    if message.location is not None:
        if not is_location_exists(message.chat.id):
            insert_location(chat_id=message.chat.id,latitude=message.location.latitude,
...
def text(message:types.Message):
    if message.chat.type=='private':
        if message.text.lower()=='продукты':
            markup=create_inline_markup(row_width=3,apple='Apple',xiaomi='Xiaomi',samsung='Samsung')
